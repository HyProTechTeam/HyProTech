...
public class SolarPage extends InteractiveCustomUIPage<SolarUpgradeEvent> {
    ...
    public SolarPage(
        ...
        super(playerRef, CustomPageLifetime.CanDismiss, SolarUpgradeEvent.CODEC);
    ...
    @Override
    public void handleDataEvent(Ref<EntityStore> playerRef, Store<EntityStore> store, SolarUpgradeEvent data) {
    ...
    private boolean handleUpgrade(Ref<EntityStore> playerRef, Store<EntityStore> store) {
        ...
        int currentTier = SolarUpgradeConfig.clampTier(node.getSolarTier());
        if (!SolarUpgradeConfig.hasNextTier(currentTier)) {
            sendPlayerMessage(playerRef, store, "Solar panel is already at max tier.");
            return false;
        }
        ...
        SolarUpgradeConfig.Requirement[] requirements =
                SolarUpgradeConfig.getUpgradeRequirements(currentTier);
        List<ItemStack> stacks = new ArrayList<>(requirements.length);
        for (SolarUpgradeConfig.Requirement requirement : requirements) {
        ...
        node.setSolarTier(nextTier);
        applySolarTier(node, nextTier);
        ...
        sendPlayerMessage(
                playerRef,
                store,
                "Upgraded solar panel to " + SolarUpgradeConfig.getTierName(nextTier) + ".");
        update(node, true);
        return true;
    }
    ...
    private void update(EnergyNodeComponent node, boolean force) {
        ...
        boolean changed = updateForNode(update, node, inventory);
        if (changed) {
            sendUpdate(update);
        } else if (force) {
            sendUpdate(new UICommandBuilder());
    ...
    private boolean updateForWorld(UICommandBuilder update, World world, ItemContainer inventory) {
        ...
        return updateForNode(update, node, inventory);
    }

    private boolean updateForNode(UICommandBuilder update, EnergyNodeComponent node, ItemContainer inventory) {
        boolean changed = false;

        int tier = SolarUpgradeConfig.clampTier(node.getSolarTier());
        if (tier != lastTier) {
            update.set("#SolarTier.Text", "Tier: " + SolarUpgradeConfig.getTierName(tier));
            lastTier = tier;
            changed = true;
        }
        ...
        int generation = node.getGeneration();
        if (generation != lastGeneration) {
            update.set("#SolarGeneration.Text",
                    "Output: " + EnergyUnits.formatWatts(generation));
    ...
    private String updateUpgradePanel(UICommandBuilder update, int tier, ItemContainer inventory) {
        boolean hasNextTier = SolarUpgradeConfig.hasNextTier(tier);
        String nextText = hasNextTier
                ? "Next: " + SolarUpgradeConfig.getTierName(tier + 1)
                : "Next: Max";
        update.set("#SolarNextTier.Text", nextText);

        String statsText = hasNextTier
                ? "Capacity: " + SolarUpgradeConfig.getCapacityForTier(tier + 1)
                        + " J | Output: " + EnergyUnits.formatWatts(
                                SolarUpgradeConfig.getGenerationForTier(tier + 1))
                : "Max tier reached.";
        update.set("#SolarUpgradeStats.Text", statsText);

        SolarUpgradeConfig.Requirement[] requirements =
                hasNextTier ? SolarUpgradeConfig.getUpgradeRequirements(tier)
                        : new SolarUpgradeConfig.Requirement[0];
        ...
        for (int i = 0; i < requirements.length; i++) {
            SolarUpgradeConfig.Requirement requirement = requirements[i];
    ...
    private void showNoNode(UICommandBuilder update) {
        update.set("#SolarTier.Text", "Tier: -");
        update.set("#SolarEnergy.Text", "Energy: -");
        update.set("#SolarGeneration.Text", "Output: -");
        update.set("#SolarNextTier.Text", "Next: -");
        update.set("#SolarUpgradeStats.Text", "");
        update.set("#UpgradeButton.Text", "No Panel");
        update.set("#UpgradeReqEmpty.Text", "No solar panel found.");
    ...
    private void applySolarTier(EnergyNodeComponent node, int tier) {
        int capacity = SolarUpgradeConfig.getCapacityForTier(tier);
        node.setCapacity(capacity);
        if (node.getEnergy() > capacity) {
            node.setEnergy(capacity);
        }
        node.setGeneration(SolarUpgradeConfig.getGenerationForTier(tier));
        node.setMaxTransfer(SolarUpgradeConfig.getMaxTransferForTier(tier));
    }
