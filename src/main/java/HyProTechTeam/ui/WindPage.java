...
public class WindPage extends InteractiveCustomUIPage<WindUpgradeEvent> {
    ...
    public WindPage(
        ...
        super(playerRef, CustomPageLifetime.CanDismiss, WindUpgradeEvent.CODEC);
    ...
    @Override
    public void handleDataEvent(Ref<EntityStore> playerRef, Store<EntityStore> store, WindUpgradeEvent data) {
    ...
    private boolean handleUpgrade(Ref<EntityStore> playerRef, Store<EntityStore> store) {
        ...
        int currentTier = WindUpgradeConfig.clampTier(node.getWindTier());
        if (!WindUpgradeConfig.hasNextTier(currentTier)) {
            sendPlayerMessage(playerRef, store, "Wind turbine is already at max tier.");
            return false;
        }
        ...
        WindUpgradeConfig.Requirement[] requirements =
                WindUpgradeConfig.getUpgradeRequirements(currentTier);
        List<ItemStack> stacks = new ArrayList<>(requirements.length);
        for (WindUpgradeConfig.Requirement requirement : requirements) {
        ...
        node.setWindTier(nextTier);
        applyWindTier(node, nextTier);
        ...
        sendPlayerMessage(
                playerRef,
                store,
                "Upgraded wind turbine to " + WindUpgradeConfig.getTierName(nextTier) + ".");
        update(node, true);
        return true;
    }
    ...
    private boolean updateForNode(UICommandBuilder update, EnergyNodeComponent node, ItemContainer inventory) {
        ...
        if (tier != lastTier) {
            update.set("#WindTier.Text", "Tier: " + WindUpgradeConfig.getTierName(tier));
            lastTier = tier;
            changed = true;
        }
        ...
        if (generation != lastGeneration) {
            update.set("#WindGeneration.Text",
    ...
    private String updateUpgradePanel(UICommandBuilder update, int tier, ItemContainer inventory) {
        boolean hasNextTier = WindUpgradeConfig.hasNextTier(tier);
        String nextText = hasNextTier
                ? "Next: " + WindUpgradeConfig.getTierName(tier + 1)
                : "Next: Max";
        update.set("#WindNextTier.Text", nextText);

        String statsText = hasNextTier
                ? "Capacity: " + WindUpgradeConfig.getCapacityForTier(tier + 1)
                        + " J | Output: " + EnergyUnits.formatWatts(
                                WindUpgradeConfig.getGenerationForTier(tier + 1))
                : "Max tier reached.";
        update.set("#WindUpgradeStats.Text", statsText);

        WindUpgradeConfig.Requirement[] requirements =
                hasNextTier ? WindUpgradeConfig.getUpgradeRequirements(tier)
                        : new WindUpgradeConfig.Requirement[0];
        ...
        for (int i = 0; i < requirements.length; i++) {
            WindUpgradeConfig.Requirement requirement = requirements[i];
    ...
    private void showNoNode(UICommandBuilder update) {
        update.set("#WindTier.Text", "Tier: -");
        update.set("#WindEnergy.Text", "Energy: -");
        update.set("#WindGeneration.Text", "Output: -");
        update.set("#WindNextTier.Text", "Next: -");
        update.set("#WindUpgradeStats.Text", "");
        update.set("#UpgradeButton.Text", "No Turbine");
        update.set("#UpgradeReqEmpty.Text", "No wind turbine found.");
    ...
    private void applyWindTier(EnergyNodeComponent node, int tier) {
        int capacity = WindUpgradeConfig.getCapacityForTier(tier);
        node.setCapacity(capacity);
        if (node.getEnergy() > capacity) {
            node.setEnergy(capacity);
        }
        node.setGeneration(WindUpgradeConfig.getGenerationForTier(tier));
        node.setMaxTransfer(WindUpgradeConfig.getMaxTransferForTier(tier));
    }
